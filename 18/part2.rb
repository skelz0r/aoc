...
def deep_set(positions, v)
  # p "Will set #{v} at position #{positions}"
...
# pair1='[[2,[[7,7],7]],[[5,8],[[9,3],[0,2]]]]'
# pair2='[[[0,[5,8]],[[1,7],[9,6]]],[[4,[1,2]],[[1,4],2]]]'
#
# @current_pair = add(eval(pair1), eval(pair2))
# handle_actions
# p @current_pair
...
p @data.permutation(2).count

@data.permutation(2).each do |pairs|
  x = eval(pairs[0])
  y = eval(pairs[1])

  @current_pair = add(x, y)
  handle_actions

  m = compute_magnitude(@current_pair)
